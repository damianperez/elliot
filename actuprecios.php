...
foreach ( $TODOS as $prod=>$data ) 
        {
            ...
            $ID_PRODUCTO= $data->id;
            $listado= Art($params['sku']);
            if ( ( !$listado ) ||  $listado->result <> "OK") 
            echo 'No hay resultados';
           // echo $data->sku.'<br>';
            foreach ( $listado->records as $item )
                {
                $params['regular_price'] = $item->Precio_final;
                if ( $item->Precio_final < 100 || $item->Stock_actual == '0' || $item->Stock_minimo == '0' )
                {   $params['status'] = 'draft';
                    $params['in_stock'] = false;
                   
                }
                $params['stock_quantity'] = $item->Stock_actual;
                /****
                 * status). Options: draft, pending, private and publish
                * in_stock
                */
                $woocommerce->put("products/$ID_PRODUCTO", $params);
                echo $ID_PRODUCTO. ' '.$params['sku'].' '.$item->Precio_final.'<br>';
                }
        }
